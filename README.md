...
- 增加明确执行意图与输入归属门禁：仅调用 Skill 不触发生成，工作区旧材料未经本轮确认不得使用。
- `run_v11_skill_checks.py` 成为当前 Harness；`run_v1_skill_checks.py` 保留为单向兼容入口。
- 模板包 V1.1 将上传的 7 页 LOOP 大会 PPTX 注册为 `loop-orange-white` 的直接可选资产；排版几何阈值与现有 PPTX/HTML 检查算法保持 V1。
...
- 支持 `white`、`light`、`dark`、`tgo-daily`、`loop-orange-white` 模板；其中 `loop-orange-white` 可直接选择“LOOP 大会”。
- 内置九类视觉风格，可按页组合。
...
python -B "$SKILL_DIR/scripts/create_generation_log.py" --mode create --source-authority current_turn --title "演示稿标题" --dry-run

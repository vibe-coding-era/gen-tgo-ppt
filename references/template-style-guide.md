# Template Style Guide

This legacy entrypoint has been split for progressive loading.

Use `references/design/index.md` first, then load only the required reusable `design.md` files:

- Shared rules: `references/design/shared/design.md`
- GTLC大会: `references/design/scenarios/gtlc-conference/design.md`
- TGO日常活动分享: `references/design/scenarios/tgo-daily-sharing/design.md`
- White template: `references/design/templates/white/design.md`
- Light template: `references/design/templates/light/design.md`
- Dark template: `references/design/templates/dark/design.md`

Do not treat this file as the full style guide. It remains only for backward-compatible discovery.

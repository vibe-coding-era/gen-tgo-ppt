...
description: 生成或改造 TGO鲲鹏会 / GTLC 风格 PPT 与 HTML 演示稿。适用于从 PPT、PDF、Markdown、HTML、粘贴内容或既有演示稿生成、套版、修复、校验 TGO/GTLC 演示材料；必须使用 V1 工作流、渐进式加载独立规则文件、生成日志、标题页后嘉宾介绍页、末尾感谢聆听页、排版安全检查、逐页渲染校对，以及“生成内容/检查风格/检查文字”职责分离。
---
...
- 执行机制：优先让不同规则文件由不同子智能体独立执行或检查，并按 SSOT 原则合并交接内容；工具不支持时，必须在日志和 SSOT 中记录限制，改用分离自检。
...
6. 每加载一个 rule 文件，就按 `references/rule-subagents.md` 分配规则检查职责；交接按 `references/rule-ssot-handoff.md` 合并。
...
10. 生成后必须运行 `scripts/check_pptx_layout.py` 或 `scripts/check_html_layout.py`，并渲染/预览逐页复核。
...
- 已为不同 rule 分配规则检查职责并形成 SSOT；工具不支持独立子智能体时已在 SSOT 和日志里说明。
- PPTX 已通过 `scripts/check_pptx_layout.py`，且没有未处理的 `FAIL`。
- 已渲染或预览每一页，并修复异常换行、文本溢出、裁切、重叠、Logo/页脚碰撞和固定页错误。
- 若可用，`生成内容`、`检查风格`、`检查文字` 使用不同子智能体；不可用时在日志和最终回复里说明替代自检方式。

## 维护约束

- 不得改写、删除、弱化或移动“重要说明和介绍”。
- 新增规则优先写入 `references/rule-*.md` 或已有对应规则文件，不要把细则堆回 `SKILL.md`；新增 rule 时必须同步补充规则职责和 SSOT 交接要求。
- 修改版本或模板体系时同步更新 `README.md`、`agents/openai.yaml`、`references/template-manifest.json`、`references/generation-log.md` 和 `scripts/create_generation_log.py`。

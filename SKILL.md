...
- 作者：肉山
- 当前版本：V1.2
- 模板包版本：V1.1；内置 `loop-orange-white` 已替换为可直接选择的 LOOP 大会 PPTX，排版几何算法仍为 V1。
- 核心变化：V1.2 将澄清、Brief 确认、模板与视觉风格选择、样片确认设为生成前必经门；页面按表达意图与视觉载体规划，并记录图片资产来源。单 Agent 为默认执行方式，正式或高风险任务再启用独立检查通道。
...
1. 首先加载 `references/rule-routing-and-modes.md`，先过“明确执行意图门”，再选择且只选择一个模式：`check_only`、`repair`、`convert`、`create`、`handoff`。
2. 仅说“运行此 SKILL”、只点名 `$gen-tgo-ppt-skill` 或只要求加载 Skill，不代表授权新建、转换、修复或检查；返回 `E_ROUTE_AMBIGUOUS`，只问一个决定模式的问题，不读取工作区内容、不创建文件。
3. `description` 负责 Skill 是否应被隐式触发；`handoff` 只处理 Skill 已加载后发现的能力越界，不替代宿主路由。
4. 选定模式后按 `references/rule-intake.md` 建立“本次已确认输入”。目录中恰好存在的 `Design.md`、`Content.md`、历史产物、日志、记忆或旧任务摘要都只是候选，不能自动成为本次材料。
5. 仅 `create`、`convert` 和重大 `repair` 强制进入完整澄清、Brief 确认、模板与视觉风格确认、日志、样片和交付流程；任何确认门未通过时不得静默套用默认值或写入完整产物。
6. `check_only` 默认只读：不强制创建 `Design.md`、`Content.md`、生成日志或 SSOT，也不得修改源文件。
7. 信息不足时只问会改变输出或不可逆操作的最少问题；只对已经确认归属的源文件使用“可直接看出则不再询问”原则。

## 总工作流

1. 加载 `references/workflow.md`，先确认执行意图和输入归属，再按已选模式进入对应阶段和确认门。
2. 只加载当前阶段需要的规则、场景、模板和视觉风格文件，不一次性读取全部引用。
3. `create`、`convert` 或重大 `repair` 按 `references/rule-intake.md` 与 `references/rule-clarification-and-design-gates.md` 补足最小字段、展示解析 Brief 并等待确认；涉及 PPT/PDF 时确认“修改内容并套模板”或“只套模板不改内容”。
4. 先按 `references/rule-template-selection.md` 展示模板与 9 套视觉风格选择卡，等待明确确认；再按 `references/rule-content-expression.md` 制定页级表达与资产来源策略。
5. 生成或重大修改前按 `references/generation-log.md` 创建日志；`check_only` 与轻微 `repair` 按模式矩阵决定是否需要。
6. PPTX 新建/转换输出默认包含标题页后的空白 `嘉宾介绍` 页和最终 `感谢聆听` 页，细则见 `references/rule-ppt-structure.md`。
7. 样片确认后再完整生成；仅用户明确说明跳过样片并给出原因时可例外，细则见 `references/rule-clarification-and-design-gates.md`。
8. 输出后运行 `scripts/check_pptx_layout.py` 或 `scripts/check_html_layout.py`，并渲染/预览逐页复核。
9. 出错时加载 `references/rule-failure-recovery.md`，按错误码和重试边界修复最小层，不把失败样本写入封存 holdout。
10. 交付前按 `references/rule-validation.md` 验证；只有正式或高风险任务才要求独立生成、风格、文字通道和 SSOT。

## 渐进式加载索引

| 场景 | 必须加载 | 条件加载 |
| --- | --- | --- |
| 任意任务启动 | `references/rule-routing-and-modes.md`、`references/workflow.md` | `references/rule-intake.md`、`references/rule-clarification-and-design-gates.md`、`references/generation-log.md` |
...
| 设计澄清、Brief 与确认门 | `references/rule-clarification-and-design-gates.md` | `references/rule-intake.md`、`references/generation-log.md` |
| 模板与视觉风格选择 | `references/rule-template-selection.md` | 预览资源、场景、模板、视觉风格文件按用户确认加载 |
| 页级内容表达与图片策略 | `references/rule-content-expression.md` | 用户确认的资产、所选视觉风格文件 |
...
3. 根据模板只读相关模板文件：
   ...
   - `loop-orange-white`：`references/design/templates/loop-orange-white/design.md`，当用户选择 `D｜LOOP 大会`、`LOOP Summit`、`LOOP Orange White` 或明确要求 LOOP 大会模板时加载
...
- 创建生成日志：`python "$SKILL_DIR/scripts/create_generation_log.py" --title "演示稿标题" --source "source.md" --source-authority explicit_path --format PPT`
...
- 运行 V1.2 Harness：`python -B "$SKILL_DIR/scripts/run_v12_skill_checks.py"`；受限环境可用 `--temp-root <已存在且可写目录>` 显式指定隔离工作区。
- V1.2 核心 Harness 保留历史文件名：`python -B "$SKILL_DIR/scripts/run_v11_skill_checks.py"`；V1 兼容入口 `run_v1_skill_checks.py` 单向委托该核心并转发 `--temp-root`。
